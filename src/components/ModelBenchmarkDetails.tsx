...
export function ModelBenchmarkDetails({ modelId, modelName, benchmarks, onClose }: ModelBenchmarkDetailsProps) {
  ...
  const getCategoryColor = (category: string) => {
    const colors = {
      ...
      multilingual: 'bg-indigo-100 text-indigo-800',
        ...
        <div className="p-4 overflow-y-auto max-h-[60vh]">
          ...
          <div className="space-y-4">
            {Object.entries(benchmarksBySource).map(([sourceName, sourceBenchmarks]) => (
              <div key={sourceName} className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center">
                    <h4 className="text-sm font-semibold text-gray-900">{sourceName}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {sourceBenchmarks.map((benchmark) => (
                    <div key={benchmark.id} className="bg-gray-50 rounded p-3">
